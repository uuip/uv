...
pub(crate) async fn download(
    ...
    // 3. Resolve the interpreter without requiring a project environment. We only need
    //    the interpreter to derive markers and tags. We discover it unconditionally
    //    (regardless of `--frozen`) because the discovery doesn't depend on the lock;
    //    this mirrors how `uv sync` resolves its environment before `LockMode` is computed.
